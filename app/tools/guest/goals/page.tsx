...
export default function GoalsPage() {
  ...
  const handleAddMoney = async () => {
    ...
    const newGoals = goals.map(g => g.id === addMoneyModal.id ? { ...g, current_amount: newCurrent, status: newStatus as 'active' | 'completed' | 'paused' } : g)
    setGoals(newGoals)
    localStorage.setItem('qv-guest-goals', JSON.stringify(newGoals))
  ...
  const handleToggleStatus = async (goal: Goal) => {
    ...
    const newGoals = goals.map(g => g.id === goal.id ? { ...g, status: newStatus as 'active' | 'completed' | 'paused' } : g)
    setGoals(newGoals)
    localStorage.setItem('qv-guest-goals', JSON.stringify(newGoals))
  ...
  return (
    <>
          ...
          {loading ? (
            ...
            <>
              {goals.map((goal) => {
                ...
                return (
                  <div key={goal.id} className={`glass-card rounded-2xl p-6 flex flex-col relative overflow-hidden group ${isCompleted ? 'border-[#fee089]/20' : ''}`}>
                        ...
                        {goal.deadline && (
                          <div className="flex gap-2">
                            ...
                            <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                              new Date(goal.deadline) < new Date() ? 'bg-[rgba(255,68,51,0.15)] text-[#ff4433]' : 'bg-[rgba(var(--color-primary-rgb),0.1)] text-[#c0c1ff]'
                            }`}>
                    ...
                    <h3 className="font-headline-lg text-xl font-bold text-[var(--text-primary)] mb-1 z-10">{goal.title}</h3>
                    ...
                    <div className="flex flex-col gap-3 mb-8 z-10">
                      <div className="flex justify-between items-end">
                        <div className="flex flex-col">
                          <span className="font-mono text-2xl font-bold text-[var(--text-primary)] leading-none mb-1">{formatCurrency(goal.current_amount)}</span>
                      ...
                      {!isCompleted ? (
                        <>
                          <button
                            ...
                            className="flex-1 py-2.5 rounded-full bg-transparent border border-white/20 text-[var(--text-primary)] hover:bg-white/5 transition-all font-body-sm text-sm flex items-center justify-center gap-2 group-hover:border-[#c0c1ff]/50"
                          >
              ...
              >
                ...
                <h3 className="font-headline-lg text-lg font-bold text-[var(--text-primary)] mb-2">Track New Goal</h3>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-[var(--bg-canvas)]/80 z-50 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
          <div className="glass-modal w-full max-w-md p-6 flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <div>
              <h3 className="text-xl font-bold text-[var(--text-primary)]">Add New Goal</h3>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                className="px-5 py-2.5 rounded-full text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-colors"
                disabled={isAdding}
              >Cancel</button>
              <button
                onClick={handleAddGoal}
                disabled={!addForm.title || !addForm.target || isAdding}
                className="px-5 py-2.5 rounded-full text-sm font-medium bg-[#c0c1ff] text-[var(--bg-canvas)] hover:shadow-[0_0_15px_rgba(var(--color-primary-rgb),0.3)] disabled:opacity-50 disabled:pointer-events-none transition-all"
              >
        ...
        <div className="fixed inset-0 bg-[var(--bg-canvas)]/80 z-50 flex items-center justify-center p-4" onClick={() => setAddMoneyModal(null)}>
          <div className="glass-modal w-full max-w-sm p-6 flex flex-col gap-5 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-end gap-3">
              <button onClick={() => setAddMoneyModal(null)} className="px-5 py-2.5 rounded-full text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5 transition-colors">Cancel</button>
              <button onClick={handleAddMoney} disabled={!addMoneyAmount} className="px-5 py-2.5 rounded-full text-sm font-medium bg-[#c0c1ff] text-[var(--bg-canvas)] hover:shadow-[0_0_15px_rgba(var(--color-primary-rgb),0.3)] disabled:opacity-50 disabled:pointer-events-none transition-all">
                Add Funds
              </button>
            </div>
          </div>
      ...
      {/* Toast */}
      {toastMsg && (
        <div className="fixed bottom-6 right-6 z-50 glass-modal px-5 py-3 text-sm text-[var(--text-primary)] animate-in slide-in-from-bottom-5 duration-300">
          {toastMsg}
        </div>
      )}
    </ToolLayout>
