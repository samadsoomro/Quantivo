...
export default function SubscriptionsPage() {
  ...
  const [form, setForm] = useState({ name: '', amount: '', billing_cycle: 'monthly' as 'weekly' | 'monthly' | 'yearly', next_renewal_date: '', category: 'Entertainment', color: 'var(--color-primary)' })
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--color-primary-hover)', margin: 0 }}>Subscriptions</h1>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', margin: '4px 0 0' }}>Never get surprised by a renewal.</p>
        </div>
        <button onClick={() => setShowAdd(true)} style={{ background: 'var(--color-primary)', color: '#1000a9', border: 'none', borderRadius: '999px', padding: '10px 20px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>+ Add Subscription</button>
      </div>

      {/* Summary cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ['Monthly Cost', fmt(totalMonthly), 'var(--color-primary)'],
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
        {(['monthly', 'yearly'] as const).map(b => (
          <button key={b} onClick={() => setBilling(b)}
            style={{ padding: '6px 16px', borderRadius: '999px', border: '1px solid', borderColor: billing === b ? 'var(--color-primary)' : 'var(--border-color)', background: billing === b ? 'rgba(var(--color-primary-rgb),0.1)' : 'transparent', color: billing === b ? 'var(--color-primary)' : 'var(--text-muted)', cursor: 'pointer', fontSize: '13px', fontWeight: 500, textTransform: 'capitalize' }}>
            {b}
          </button>
        ))}
      </div>
      ...
      {showAdd && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '16px' }}>
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '440px' }}>
            <h2 style={{ color: 'var(--color-primary-hover)', fontSize: '20px', fontWeight: 700, marginBottom: '24px' }}>Add Subscription</h2>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
              ...
              <button onClick={addSub} disabled={!form.name || !form.amount || saving} style={{ flex: 2, padding: '11px', borderRadius: '8px', background: 'var(--color-primary)', color: '#1000a9', border: 'none', fontWeight: 700, cursor: 'pointer', fontSize: '14px', opacity: !form.name || !form.amount ? 0.5 : 1 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '80px 24px', background: 'rgba(18,33,49,0.4)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <h3 style={{ color: 'var(--color-primary-hover)', fontSize: '20px', fontWeight: 600, marginBottom: '8px' }}>No subscriptions tracked</h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '24px' }}>Add your recurring services to track spending</p>
          <button onClick={() => setShowAdd(true)} style={{ background: 'var(--color-primary)', color: '#1000a9', border: 'none', borderRadius: '999px', padding: '10px 24px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>Add first subscription</button>
        </div>
      ) : (
            ...
            return (
              <div key={sub.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '20px', transition: 'all 0.2s' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <p style={{ fontSize: '15px', fontWeight: 600, color: 'var(--color-primary-hover)', margin: 0 }}>{sub.name}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ fontSize: '22px', fontWeight: 700, color: 'var(--color-primary)', fontFamily: 'JetBrains Mono, monospace', margin: 0 }}>{fmt(displayAmt)}</p>
                    <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: '2px 0 0' }}>/{billing === 'yearly' ? 'year' : 'month'}</p>
                  </div>
                  {daysUntil !== null && (
                    <div style={{ background: daysUntil <= 7 ? 'rgba(255,180,171,0.15)' : 'rgba(var(--color-primary-rgb),0.08)', borderRadius: '8px', padding: '4px 10px' }}>
